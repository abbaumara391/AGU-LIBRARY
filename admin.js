...
function getId(p){return p.id||p.user_id||p.student_id||"";}
...
async function adminMfaGate(){
 if(mfaOpen)return true; const d=getDB();
 const stampKey="AGU_ADMIN_MFA_VERIFIED_AT", last=Number(localStorage.getItem(stampKey)||0);
 if(Date.now()-last<12*60*60*1000){mfaOpen=true;return true;}
 let factors;
 try{factors=await d.auth.mfa.listFactors();}catch(e){console.error(e);alert("Administrator MFA could not be checked: "+(e.message||"Unknown error"));return false;}
 const totp=(factors.data?.totp||[]).find(f=>f.status==="verified")||factors.data?.totp?.[0];
 if(!totp){alert("Administrator Google Authenticator is not enrolled. Open Supabase Auth → MFA and enroll a TOTP authenticator for this administrator before using the Admin dashboard.");return false;}
 const overlay=document.createElement("div");overlay.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.58);z-index:99999;display:grid;place-items:center;padding:20px";
 overlay.innerHTML=`<div style="width:min(440px,100%);background:#fff;border-radius:22px;padding:28px;box-shadow:0 20px 60px rgba(0,0,0,.25)"><h2 style="margin-top:0">🔐 Admin verification</h2><p style="color:#718079;line-height:1.6">Enter the current 6-digit Google Authenticator code to continue.</p><input id="aguMfaCode" inputmode="numeric" maxlength="6" autocomplete="one-time-code" placeholder="000000" style="width:100%;padding:14px;border:1px solid #cfe5da;border-radius:10px;font-size:20px;letter-spacing:5px;text-align:center"><button id="aguMfaVerify" style="width:100%;margin-top:12px;border:0;border-radius:10px;padding:13px;background:#087a4b;color:#fff;font-weight:900" type="button">Verify & Continue</button><p id="aguMfaError" style="color:#b42323;font-size:13px"></p></div>`;
 document.body.appendChild(overlay);mfaOpen=true;
 $("aguMfaVerify").onclick=async()=>{const code=($("aguMfaCode").value||"").replace(/\D/g,"");if(code.length!==6){$("aguMfaError").textContent="Enter the 6-digit code.";return;}$("aguMfaVerify").disabled=true;try{const r=await d.auth.mfa.challengeAndVerify({factorId:totp.id,code});if(r.error)throw r.error;localStorage.setItem(stampKey,String(Date.now()));overlay.remove();await finishAdmin();}catch(e){$("aguMfaError").textContent=e.message||"Verification failed.";$("aguMfaVerify").disabled=false;}};
 $("aguMfaCode").focus();return false;
}

async function loadStudents(){
 const d=getDB(),list=$("aguStudentList");
 if(!list)return;
 list.innerHTML='<div class="empty">Loading students...</div>';
 try{
  /*
   * student_profiles uses the profile primary key as the student's
   * Supabase user id. Do not require created_at because that column
   * is not guaranteed to exist.
   */
  let r=await d.from("student_profiles").select("*");
  if(r.error)throw r.error;
  students=Array.isArray(r.data)?r.data:[];

  /* Keep the list useful even when the table has no timestamp column. */
  students.sort((a,b)=>{
   const ad=String(a.created_at||a.updated_at||a.createdAt||"");
   const bd=String(b.created_at||b.updated_at||b.createdAt||"");
...
function updateDigitalBookFields(){const type=$("resourceType"),normal=$("normalFileField"),digital=$("digitalBookFields"),file=$("file");if(!type)return;const isDigital=type.value==="digital_book";digital?.classList.toggle("hidden",!isDigital);normal?.classList.toggle("hidden",isDigital);if(file)file.required=!isDigital;if(isDigital&&$("bookEntry")&&!$("bookEntry").value)$("bookEntry").value="index.html";}
...
function renderResources(){const list=$("aguResourceList"),q=($("aguResourceSearch")?.value||"").toLowerCase().trim();if(!list)return;const rows=resources.filter(r=>[r.title,r.name,r.subject,r.category,r.class_level,r.type,r.folder_path].join(" ").toLowerCase().includes(q));if(!rows.length){list.innerHTML='<div class="empty">No resources found.</div>';return;}list.innerHTML=rows.slice(0,200).map((r,i)=>{const url=r.file_url||r.url||"";const key=r.id||r.storage_path||r.file_url||r.url||`${r.title||r.name||"resource"}-${i}`;return `<div class="item"><div class="item-text"><strong>${esc(r.title||r.name||"Untitled Resource")}</strong><div class="small">${esc(r.subject||r.category||"")} ${r.class_level?"• "+esc(r.class_level):""} ${r.type?"• "+esc(r.type):""}</div><div class="small">${esc(r.folder_path||r.storage_path||"")}</div></div><div style="display:flex;gap:8px;flex-wrap:wrap">${url?`<a class="btn blue" href="${esc(url)}" target="_blank" rel="noopener">Open</a>`:""}<button class="btn delete-resource agu-delete-resource" data-resource-key="${esc(key)}" type="button">🗑 Delete</button></div></div>`;}).join("");list.querySelectorAll(".agu-delete-resource").forEach(b=>b.onclick=()=>deleteResource(b.dataset.resourceKey));}

async function deleteResource(resourceKey){const d=getDB();const resource=resources.find(r=>String(r.id??r.storage_path??r.file_url??r.url??`${r.title||r.name||""}`)===String(resourceKey));if(!resource){showMessage("Resource could not be found. Refresh the resource list and try again.","error");return;}const title=resource.title||resource.name||"this resource";if(!confirm(`Delete "${title}"?\n\nThis action removes the published resource from AGULIBRARY. It cannot be undone.`))return;const list=$("aguResourceList");const buttons=list?Array.from(list.querySelectorAll(".agu-delete-resource")):[];const button=buttons.find(b=>String(b.dataset.resourceKey)===String(resourceKey));if(button)button.disabled=true;try{showMessage("Administrator verification required for deletion.","success");const verified=await adminMfaGate();if(!verified){if(button)button.disabled=false;return;}showMessage(`Deleting "${title}"...`,"success");let storageError=null;const storagePath=resource.storage_path||resource.path||"";if(storagePath){const sr=await d.storage.from(BUCKET).remove([storagePath]);if(sr.error)storageError=sr.error;}let dbResult;if(resource.id!==undefined&&resource.id!==null&&String(resource.id)!==""){dbResult=await d.from(TABLE).delete().eq("id",resource.id);}else if(storagePath){dbResult=await d.from(TABLE).delete().eq("storage_path",storagePath);}else if(resource.file_url||resource.url){const field=resource.file_url?"file_url":"url";dbResult=await d.from(TABLE).delete().eq(field,resource[field]);}else{throw new Error("This resource has no safe database identifier. Refresh the list and try again.");}if(dbResult.error)throw dbResult.error;if(storageError){showMessage(`"${title}" was removed from the library, but its stored file could not be deleted: ${storageError.message||"Storage error"}.`,"error");}else{showMessage(`"${title}" was deleted successfully.`,"success");}await loadResources();}catch(e){console.error(e);showMessage(e.message||"Resource deletion failed. No further changes were made by the dashboard.","error");if(button)button.disabled=false;}}

async function uploadFile(e){e.preventDefault();const d=getDB(),title=$("title").value.trim(),category=$("category").value.trim(),type=$("resourceType").value,status=$("uploadStatus"),button=$("uploadButton");if(!title){status.textContent="Please enter a resource title.";return;}button.disabled=true;status.textContent="Preparing...";try{if(type==="digital_book"){let bookPath=$("bookPath")?.value.trim()||"",bookEntry=$("bookEntry")?.value.trim()||"index.html";if(!bookPath){status.textContent="Please enter the digital book folder path.";return;}bookPath=bookPath.replace(/^[\/\\]+|[\/\\]+$/g,"");if(!bookPath){status.textContent="Please enter the digital book folder path.";return;}bookEntry=bookEntry.replace(/^[\/\\]+/,"");if(!bookEntry)bookEntry="index.html";const digitalBookURL="/"+bookPath.replace(/[\/\\]+$/,"")+"/"+bookEntry;const payload={title,subject:category,type:"digital_book",file_url:digitalBookURL,name:title,category,url:digitalBookURL};let result=await d.from(TABLE).insert(payload);if(result.error){const fallback={name:title,category,type:"digital_book",url:digitalBookURL};result=await d.from(TABLE).insert(fallback);if(result.error)throw result.error;}status.textContent="✅ Digital book published successfully.";showMessage("Digital book published successfully.","success");$("uploadForm").reset();updateDigitalBookFields();await loadResources();return;}const file=$("file").files[0];if(!file){status.textContent="Please select a file.";return;}status.textContent="Uploading...";const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"_");const filename=`resources/${Date.now()}-${safe}`;const up=await d.storage.from(BUCKET).upload(filename,file,{upsert:false});if(up.error)throw up.error;const pub=d.storage.from(BUCKET).getPublicUrl(filename);const url=pub.data.publicUrl;const payload={title,subject:category,type,file_url:url,storage_path:filename,name:title,category,url};let result=await d.from(TABLE).insert(payload);if(result.error){const fallback={name:title,category,type,url};result=await d.from(TABLE).insert(fallback);if(result.error)throw result.error;}status.textContent="✅ Upload successful.";showMessage("Resource uploaded successfully.","success");$("uploadForm").reset();updateDigitalBookFields();await loadResources();}catch(err){console.error(err);status.textContent="❌ "+(err.message||"Upload failed.");showMessage(err.message||"Upload failed.","error");}finally{button.disabled=false;}}
...
function bind(){
 $("adminLoginForm")?.addEventListener("submit",login);$("uploadForm")?.addEventListener("submit",uploadFile);$("logoutButton")?.addEventListener("click",logout);$("resourceType")?.addEventListener("change",updateDigitalBookFields);updateDigitalBookFields();
 ...
 $("adminMfaReset")?.addEventListener("click",()=>{localStorage.removeItem("AGU_ADMIN_MFA_VERIFIED_AT");mfaOpen=false;alert("Administrator verification has been reset. The next protected action will request your Authenticator code.");});
...
window.AGU_ADMIN={loadStudents,loadResources,sendNotification,uploadFile,deleteResource,logout};
})();
